...
- No forbidden exercises (check preferences KB for specific injury constraints)
...
| Condition | Why |
|-----------|-----|
| Uses forbidden exercise (check preferences KB for specific constraints) | Injury constraint violation |
| Uses discouraged exercises (check preferences KB) | Reduce selection score instead of auto-fail |
| Date mismatch (output date ≠ input date) | Hallucination |
| Workout generated for rest/trainer day (check status KB) | Should skip this day |
...
- **+0.5:** Workout includes goal-relevant progression notes (aligned with user's primary goal from goals KB)
